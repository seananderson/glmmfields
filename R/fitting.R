...
#' @param time A character object giving the name of the time column. Leave as
#'   \code{NULL} to fit a spatial GLMM without a time element.
...
#'   Smaller values will be faster but may not adequately represent the shape of
#'   the spatial pattern.
...
#'   are normal or half-normal can be implemented by setting the first parameter
#'   in the half-t or student-t distribution to a large value.
...
#'   the CV for the Gamma or the dispersion parameter for the negative binomial.
...
#' @param prior_phi The prior on the AR parameter. Must be declared with
#'   \code{\link{student_t}}.
...
#'   approximate the normal distribution. If the value is >=1000 then a true MVN
...
#'   between random fields at time(t) and time(t-1). If the phi parameter is
#'   estimated then this argument is ignored.
...
#' @param covariance The covariance function of the Gaussian Process. One of
#'   "squared-exponential", "exponential", or "matern".
#' @param matern_kappa Optional parameter for the Matern covariance function.
#'   Optional values are 1.5 or 2.5. Values of 0.5 are equivalent to
#'   exponential.
...
#'   \code{TRUE}, then no fixed effects (B coefficients) will be estimated. In
#'   this case, \code{prior_intercept} will be used as the prior for the initial
#'   value in time.
#' @param nb_lower_truncation For NB2 only: lower truncation value. E.g. 0 for
#'   no truncation, 1 for 1 and all values above. Note that estimation is likely
#'   to be considerably slower with lower truncation because the sampling is not
#'   vectorized. Also note that the log likelihood values returned for
#'   estimating quantities like LOOIC will not be correct if lower truncation is
#'   implemented.
...
#' versions. Therefore it is important that you specify any priors that are used
#' in your model, even if they replicate the defaults in the package. It is
#' particularly important that you consider that prior on \code{gp_theta} since
#' it depends on the distance between your location points. You may need to
...
glmmfields <- function(formula, data, lon, lat,
  ...
  assert_that(obs_error[[1]] %in% c(
    "gaussian", "gamma", "poisson", "nbinom2", "binomial", "lognormal"
  ))
  ...
  if (nb_lower_truncation > 0) {
    warning(
      ...
      "tested, is likely slow, and calculation of the log likelihood for ",
      "information criteria purposes is likely to be incorrect."
    )
  }
  ...
  stan_data <- c(
    ...
    list(
      ...
      cov_func = switch(covariance,
        ...
        matern = 2L,
        stop(paste("Vovariance function", covariance, "is not defined."))
      ),
      obs_model = obs_model,
