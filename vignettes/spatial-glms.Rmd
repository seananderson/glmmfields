...
For large datasets, the predictive Gaussian process described above can be overwhelmed (as the number of knots increases, the dimensionality of the matrix needed to be inverted also increases). For this reason, we've included a nearest neighbor Gaussian process function as an option for large datasets. To illustrate an example, we'll generate a dataset with a larger number of samples (1000). Even generating the dataset is a bit slower because of the matrix inversion,

```{r}
set.seed(1)
N <- 1000 # number of data points
X <- matrix(1,nrow=N) # our design matrix
s <- sim_glmmfields(
  n_draws = 1, gp_theta = 1.5, n_data_points = N,
  gp_sigma = 0.2, sd_obs = 0.2, n_knots = 20,
  covariance = "squared-exponential", X = X
)
d <- s$dat
...
Now we'll add two arguments to the glmmfields() call. First, we'll set method = "NNGP", and second we'll specify the number of neighbors used in calculations (nngp_neighbors, defaults to 5). Increasing the number of neighbors tends to make calculations more slow (5 takes a few minutes, 10 might take more like 20 minutes), and the estimated surface more smooth.
