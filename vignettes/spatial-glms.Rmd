...
s <- sim_rrfield(n_draws = 1, gp_scale = 2, n_data_points = N,
...
Let's instead fit a spatial GLM with random fields. (Note that we are only using 2 chains and 600 iterations here so the vignette builds quickly. For final inference, you should likely use 4 or more chains and possibly more iterations.)

```{r, results='hide'}
m_spatial <- rrfield(y ~ temperature, data = d, family = Gamma(link = "log"), 
